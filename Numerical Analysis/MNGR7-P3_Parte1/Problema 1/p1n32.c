...
int main (void)
{
  int i,j,n=32,np=180; /*n nombre nodes interpolació, np nombre de punts a representar una vegada obtingut el polinomi*/
  ...
  for(j=0;j<=np;j++)
  {
      printf("\n%.8G\t%.8G\t%.8G\t%.8G\t%.8G",evalj[j],xj[j],poli[j],fj[j],fevalj[j]);
  }
